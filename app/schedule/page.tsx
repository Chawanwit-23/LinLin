...
import { useClasses } from "@/lib/useStore";
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-pink-50 overflow-hidden">
      <div className="flex border-b border-pink-100 bg-pink-50 sticky top-0 z-10">
        <div className="shrink-0 flex items-center justify-center border-r border-pink-100" style={{ width: DAY_LABEL_W }}>
        ...
        <div className="overflow-x-auto flex-1 scrollbar-none">
          <div className="flex" style={{ width: totalTimeW }}>
            {HOURS.map((h) => (
              <div key={h} className="shrink-0 py-3 text-center text-xs font-bold text-gray-600 border-r border-pink-100 last:border-r-0" style={{ width: HOUR_WIDTH }}>
                {h}:00
              </div>
            ))}
          </div>
      ...
      <div className="flex">
        <div className="shrink-0 border-r border-pink-100" style={{ width: DAY_LABEL_W }}>
          {DAYS.map((day, i) => (
            <div key={day} className={`flex items-center justify-center px-1 border-b border-pink-50 last:border-b-0 ${i % 2 === 0 ? "bg-pink-50/40" : "bg-white"}`} style={{ height: ROW_HEIGHT }}>
        ...
        <div className="overflow-x-auto flex-1 scrollbar-none">
          <div style={{ width: totalTimeW }}>
            {DAYS.map((_, dayIdx) => {
              ...
              return (
                <div key={dayIdx} className={`relative border-b border-pink-50 last:border-b-0 ${dayIdx % 2 === 0 ? "bg-pink-50/20" : "bg-white"}`} style={{ height: ROW_HEIGHT, width: totalTimeW }}>
                  {HOURS.map((_, hi) => (
                    <div key={hi} className="absolute top-0 bottom-0 border-r border-pink-50" style={{ left: hi * HOUR_WIDTH }} />
                  ))}
                  {dayClasses.map((c) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {c.room && <p className="text-[10px] text-gray-500 truncate">{c.room}</p>}
                        <button onClick={() => onDelete(c.id)} className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 active:opacity-100 transition-opacity bg-white/80 rounded-full p-0.5">
...
export default function SchedulePage() {
  ...
  const handleAdd = async (data: Omit<ClassItem, "id">) => {
    if (!data.subject.trim()) return;
    await addClass(data);
    setShowForm(false);
  };

  const handleDelete = (id: string) => removeClass(id);
